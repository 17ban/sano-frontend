  ...
  for(const k in opt) {
    ...
    if(ret) {
      fmt = fmt.replace(ret[1], (ret[1].length == 1) ? 
        (opt[<optKey>k]) : opt[<optKey>k].padStart(ret[1].length, '0'))
    }
  }
  return fmt
}
...
export function goto(x: number, y: number, duration: number) {
  function _f(x: number, e: number = 3){
    if(0<=x && x<=1) return Math.pow(x, e)
    else if(1<x && x<=2) return 2-Math.pow((2-x), e)
    else throw new Error('x is out of range: [0, 2]')
  }
  function f(x: number, x_max: number, y_max: number, e: number = 3) {
    let x_rate = 2/x_max
    let y_rate = y_max/2
    return _f(x * x_rate, e) * y_rate
  }

  let [start_x, start_y] = [window.pageXOffset, window.pageYOffset]
